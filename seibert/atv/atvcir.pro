PRO atvcir,x,y,id,sze=sze,rad=rad,offst=offst,circ=circ
;+
; NAME:
;	ATVCIR
; PURPOSE:
...
;
;       Modified to work with atv, M. Seibert 7/1/02
;
;-
...
if (keyword_set(circ) eq 0) then atvcircle,rad,x,y	;Circles not suppressed
...
if (n_params() lt 3) then begin	   ;****** DISPLAY INDEX NUMBER ******
   ...
   if (offst lt 0) then begin	   ;Display on LEFT
      ...
      for i=0,cnt-1 do begin
         atvxyouts,x(i)+offst+1.005*rad,y(i),'-'+strtrim(i,2),$
           /dev,size=sze
      endfor
   endelse
endif else begin
...
;
   ...
   if (offst lt 0) then begin	;  Display on LEFT
      if (b(2) eq 7) then begin  ;  String array
         for i=0,cnt-1 do begin
           if (id(i) ne '') then begin    ; 
            ...
            atvxyouts,x(i)-sze*rad-(lnth*9)-8*sze+pad,y(i),$
         ...
         for i=0,cnt-1 do begin	
           if (id(i) gt -1) then begin   ;  allows skipping over elements
            if (id(i) lt 10) then begin
              atvxyouts,x(i)+2*offst-9*sze,y(i),strtrim(id(i),2)+'-',/dev,size=sze
            endif 
            if (id(i) ge 10) AND (id(i) le 99) then begin
               atvxyouts,x(i)+2*offst-12*sze,y(i),strtrim(id(i),2)+'-',/dev,$
                   size=sze
            endif
            if (id(i) gt 99) then begin	 ; gt 99
               atvxyouts,x(i)+2*offst-14*sze,y(i),strtrim(id(i),2)+'-',/dev,$
                  size=sze
            endif
           endif 
        endfor
      ...
      if (b(2) eq 7) then begin  ;  String array
         for i=0,cnt-1 do begin
           if (id(i) ne '') then begin    ; 
               atvxyouts,x(i)+offst+1.005*rad,y(i),'-'+strtrim(id(i),2),/dev,$
                  size=sze
           endif
         endfor
      endif
