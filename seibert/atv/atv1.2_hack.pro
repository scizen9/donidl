...
;       This version is 1.2, last modified 17 August 2000.
...
;
;       MHS tweked here and there.
;       changed autoscale to use sky routine and -3 to +7sigma
;
;-
;----------------------------------------------------------------------
...
;mhs - atv like the original IDL color setting
...
if (event.type EQ 0) then BEGIN   ; MHS reorder
    case event.press of
        2: atv_apphot
        4: atv_zoom, 'none', /recenter
        1: atv_showstats
        else: 
    endcase
endif
...
case uvalue of
    ...
    'reset_color': begin   
    ; set brightnews and contrast to default positions and 
    ; reset bottom 8 colors
        atv_initcolors
...
sky,main_image, sky, skysigma,/silent ; MHS auto sigma scale (Mscale)
state.min_value = (sky - 3*skysigma) > min(main_image)
state.max_value = (sky + 7*skysigma) < max(main_image)
...
state.image_min = min(main_image, /nan)
state.image_max = max(main_image, /nan)
...
if (numext EQ 0 AND naxis LT 2) then begin
    if (n_elements(fitsfilename) EQ 0) then begin
        ...
        print, 'ERROR: File is not a 2-d FITS image!'
    endelse
    return
endif
...
if ( (size(main_image))[0] NE 2 ) then begin
    mesg = 'Selected file is not a 2-D fits image!'
    tmp_result = dialog_message(mesg, /error, dialog_parent = state.base_id)
...
; Check for NaNs and convert to zero
;w = where(finite(main_image) EQ 0, count)
...
if (xten EQ 'BINTABLE') then begin
    mesg = 'File appears to be a FITS table, not an image.'
    tmp_result = dialog_message(mesg, /error, dialog_parent = state.base_id)
    cancelled = 1
    return
endif
...
r = replicate(r_vector(d-2), 2*d + dp)
g = replicate(g_vector(d-2), 2*d + dp)
b = replicate(b_vector(d-2), 2*d + dp)
...
if (state.wcstype NE 'angle') then begin 
    mesg = 'Cannot get coordinate info for this image!'
    tmp_result = dialog_message(mesg, /error, $
                                dialog_parent = state.base_id)
    return
endif
...
; this routine.
if (not(xregistered('atv', /noshow))) then userwindow = !d.window
