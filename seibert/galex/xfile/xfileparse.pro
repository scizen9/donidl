pro xfileparse, band=band, field=field, xpath=xpath, $
       ...
       clean=clean, subvisit=subvisit, pipe=pipe

;-------------------------------------       

if not keyword_set(pipe) then pipe='ops'
...
sv=''
if keyword_set(subvisit) then sv='_'+subvisit

xfile=field+sv+'-'+band+'d-x.fits'
...
if not file_exist(xpath+xfile) then begin
 ...
 if keyword_set(subvisit) then begin
  spawn,'ls '+dpath+'/'+field+sv+'-asprta.*',aspf
  cmd=cmd+' -ib '+field+sv+' -aspf '+aspf

 endif

 openw,lun,'xfilecreate.s',/get_lun
 printf,lun,pipepath+'newcfg '+pipe+' -exec \ '
 printf,lun, cmd
 printf,lun, 'exit'
 free_lun,lun
...
rrfile=field+sv+'-'+band+'d-rr.fits*'  
mcatfile=field+sv+'-xd-mcat.fits*'
    ...
    for ii=(lo-1)>0, (hi+1)<(n_elements(clock_time)-1) do begin
     if clock[ii] eq 0 then begin
       a=where(t ge expstart+ii*interval and $
               t lt expstart+(ii+1)*interval,count)
...
for j=0,n_elements(m)-1 do begin
 ...
 save,filename=ofile,t,ra,dec,rr,$
