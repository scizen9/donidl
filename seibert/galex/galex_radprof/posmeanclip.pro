PRO POSMEANCLIP, Image, Mean, Sigma, CLIPSIG=clipsig, MAXITER=maxiter, $
    CONVERGE_NUM=converge_num, VERBOSE=verbose, SUBS=subs
;+
; NAME:
;	POSMEANCLIP (derived from MEANCLIP)
;
; PURPOSE:
;	Computes an iteratively sigma-clipped mean on a data set
;       ***** clips only values greater than mean (MHS 8/31/08)*****
; 
; EXPLANATION:
;       Clipping is done about mean.
;
; CATEGORY:
;	Statistics
;
...
;                   improved doc.  RSH
;       converted to posmeanclip 8/31/08 MHS
;-
...
REPEAT BEGIN
    ...
    wsm = where((skpix-meanval) LT clipsig*sig,ct)
