pro my_imcontour, im, hdr, TYPE=type, PUTINFO=putinfo, XTITLE=xtitle,  $
      YTITLE=ytitle, SUBTITLE = subtitle, XDELTA = xdelta, YDELTA = ydelta, $
      NAME=name,ANONYMOUS_ = dummy_,_EXTRA = extra
;+
; NAME:
;	IMCONTOUR
; PURPOSE:
...
;  Draw properly curved ticks
	...
	for ll=0,n_elements(ra_grid1)-1 do begin
		oplot,cons_ra(ra_grid1[ll],xx1,astr)<xsize>0,yy1,psym=0
		oplot,cons_ra(ra_grid1[ll],xx2,astr)<xsize>0,yy2,psym=0
	endfor
	for ll=0,n_elements(dec_grid1)-1 do begin
		oplot,xx1,cons_dec(dec_grid1[ll],yy1,astr)<ysize>0,psym=0
		oplot,xx2,cons_dec(dec_grid1[ll],yy2,astr)<ysize>0,psym=0
	endfor
