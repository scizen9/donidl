pro curvimcontour, im, hdr, TYPE=type, PUTINFO=putinfo, XTITLE=xtitle,  $
      YTITLE=ytitle, SUBTITLE = subtitle, XDELTA = xdelta, YDELTA = ydelta, $
      NAME=name,ANONYMOUS_ = dummy_,_EXTRA = extra, pos=pos, thick=thick
;+
; NAME:
;	CURVIMCONTOUR (hacked imcontour for curved sky grid)
; PURPOSE:
...
;       Set TYPE=1 to measure distances in RA and DEC
;       Set TYPE=2 to to overlay a RA/DEC coordinate grid
...
;		declination.
;		
;		TYPE = 2 astronomical labeling with RA and DEC with a grid
;
...
;
;	Modified to make curved ticks and proper sky grids 
;       Type=1 (proper ticks and grid) Type=2 (proper ticks)
;
;                                            M. Seibert
;                                            C. Danforth Feb 2000
;-
...
if type NE 0 then begin                  ;RA and Dec labeling
	...
	ra_grid1= (botmin + findgen(5*nx+1)*raincr/4. - 2.*nx*raincr/4.)
;     ra_grid = (botmin + findgen(nx+1)*raincr/60.) 
;       ra_grid1= (botmin + findgen(5*nx+1)*raincr/60. - 2.*nx*raincr/60.)
     dec_grid = (leftmin + findgen(ny+1)*decincr/60.)
	dec_grid1= (leftmin + findgen(5*ny+1)*decincr/60. - 2.*ny*decincr/60.)

     ticlabels, botmin, nx+1, raincr, xlab, DELTA=xdelta, /RA
...
; Get default values of XTITLE, YTITLE, TITLE and SUBTITLE
  ...
  ; let's get equinox info
  yr = get_equinox(hdr,code)
  year=fix(yr)
  year = strmid(year,4,4)
  IF code eq -1 THEN year = 'unknown'

;  if (not keyword_set( SUBTITLE) ) and (putinfo LT 1) then $
;      subtitle = '!C' + name + ':   !4a!X= '+ strmid(ra_dec,1,13)+  $
;                 ' !4d!X=' + strmid(ra_dec,13,13) + '  (' + year + ')'
;  if (not keyword_set( SUBTITLE) ) then subtitle = !P.SUBTITLE
  ...
  if type eq 0 then begin
	 ...
	 XMINOR = xminor, YMINOR = yminor, _EXTRA = extra;,charsize=1.25
  endif
  ...
  if type NE 0 then begin
     contour,im, $
         XTICKS = nx, YTICKS = ny, POSITION=pos, XSTYLE=1, YSTYLE=1,$
         XTICKV = xpos, YTICKV = ypos, XTITLE=xtitle, YTITLE=ytitle, $
         XTICKNAME = xlab, YTICKNAME = ylab, SUBTITLE = subtitle, TICKLEN=0,$
	 XMINOR = xminor, YMINOR = yminor, _EXTRA = extra;charsize=1.25

;  Draw properly curved ticks
	xx1=(xsize/75)*findgen(2)& xx2=xsize-(xsize/75)*findgen(2)
	yy1=(xsize/75)*findgen(2)& yy2=ysize-(ysize/75)*findgen(2)
	for ll=0,n_elements(ra_grid1)-1 do begin
		oplot,cons_ra(ra_grid1[ll],xx1,astr)<xsize>0,yy1,$
                      psym=0,thick=thick
		oplot,cons_ra(ra_grid1[ll],xx2,astr)<xsize>0,yy2,$
                      psym=0,thick=thick
	endfor
	for ll=0,n_elements(dec_grid1)-1 do begin
		oplot,xx1,cons_dec(dec_grid1[ll],yy1,astr)<ysize>0,$
                      psym=0,thick=thick
		oplot,xx2,cons_dec(dec_grid1[ll],yy2,astr)<ysize>0,$
                      psym=0,thick=thick
	endfor
	if type GE 2 then begin

;  Draw grid using properly curved lines
  	 xx=(1)*findgen(xsize/1)
	 yy=(1)*findgen(ysize/1)
;     Draw RA lines (North-South gridlines)
	 for ll=0,n_elements(ra_grid1)-1 do begin
	 	oplot,cons_ra(ra_grid1[ll],xx,astr)<xsize>0.,yy,line=2
	 endfor
;     Draw DEC lines (East-West gridlines)
	 for ll=0,n_elements(dec_grid1)-1 do begin
	 	oplot,xx,cons_dec(dec_grid1[ll],yy,astr)<ysize>0.,line=2
	 endfor
	endif
  endif
