pro imsetplot, im, hdr, TYPE=type, PUTINFO=putinfo, XTITLE=xtitle,  $
      YTITLE=ytitle, SUBTITLE = subtitle, XDELTA = xdelta, YDELTA = ydelta, $
      name=name,overplot=overplot,ANONYMOUS_ = dummy_,_EXTRA = extra
;+
; NAME:
;	IMCONTOUR
; PURPOSE:
...
if type NE 0 then begin                  ;RA and Dec labeling
     ...
     xunits = font+'Right Ascension (J2000)'
     yunits = font+'Declination (J2000)'
...
;	 XMINOR = xminor, YMINOR = yminor, _EXTRA = extra, charsize=1.25
     ...
     ;xtitle = '!17Right Ascension (J2000)'
     ;ytitle = '!17Declination (J2000)'

  contour,im, $
         XTICKS = nx, YTICKS = ny, XSTYLE=1, YSTYLE=1,title=name,$
         XTICKV = xpos, YTICKV = ypos, XTITLE=xtitle, YTITLE=ytitle, $
         XTICKNAME = xlab, YTICKNAME = ylab, SUBTITLE = subtitle, $
	 XMINOR = xminor, YMINOR = yminor, _EXTRA = extra, charsize=1.25,$
         overplot = overplot ;position = [.08,.08,.95,.95]
