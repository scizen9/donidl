...
for i=0,n_elements(fuvfile)-1 do begin 
 ...
 if be or re then begin  ; if fits files exists
  ...
  print,minmax(red),minmax(green), minmax(blue)

  sky,red,   rsky, rskysigma, /circ,/silent   ; quick method for sky & sigma
  sky,blue,  bsky, bskysigma, /circ,/silent
  sky,green, gsky, gskysigma, /circ,/silent

  if rskysigma eq 0 or bskysigma eq 0 then begin

   sky=moment(red)
   rsky=sky[0] & rskysigma=sqrt(sky[1])
   sky=moment(blue)
   bsky=sky[0] & bskysigma=sqrt(sky[1])
   sky=moment(green)
   gsky=sky[0] & gskysigma=sqrt(sky[1])
  ...
  print,minmax(r),minmax(g), minmax(b)
  print, rsky,rskysigma,gsky,gskysigma,bsky,bskysigma
  print, rnsig,gnsig,bnsig

  if keyword_set(no_annotations) then goto,skipann
