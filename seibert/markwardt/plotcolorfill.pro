...
;
;  $Id: plotcolorfill.pro,v 1.4 2001/04/11 00:46:12 craigm Exp $
;
;-
...
pro plotcolorfill, x0, y0, color=col, bottom=bot, width=width, $
  ...
  if n_elements(panel) EQ 0 AND n_elements(subpanel) EQ 0 then begin
      ...
      plotpan, [min(x), max(x)], [min(y), max(y)], /nodata, $
  ...
  if NOT keyword_set(over) then begin
      ...
      plot, [min(x),max(x)], [min(y), max(y)], /nodata, /noerase, /normal, $
        xrange=xrange, yrange=yrange, xstyle=xstyle, ystyle=ystyle, $
        position=position, xlog=xlog, ylog=ylog, _extra=extra
  endif
