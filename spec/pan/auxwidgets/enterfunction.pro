; $Id: enterfunction.pro,v 1.2 2010/05/14 17:30:26 neill Exp $
;
; ENTERFUNCTION.PRO
;
...
fdir = '/Users/neill/idl/spec/pan/functions/'

widget_control,event.top,get_uvalue = pState
case sel_index of
  0: begin
                        ;print,'selection 1'
                        filename = fdir + 'SII.eq'
            if filename eq '' or filename eq ' ' then return
            thisExpr = ''
                        openr,lun,filename,/get_lun
                        readf,lun,thisExpr
  ...
  4: begin
                        ;print,'selection 5'
                        filename = fdir + 'HaNII_redshift.eq'
            if filename eq '' or filename eq ' ' then return
            thisExpr = ''
                        openr,lun,filename,/get_lun
                        readf,lun,thisExpr
...
;** MODIFY here as well
selections = ['restore_SII','restore_HaNII','restore_CaII','restore_NaI','restore_HaNII_redshift']
sel_index=0
