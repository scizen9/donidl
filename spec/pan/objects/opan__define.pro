...
;       Modified (fiddled with) by JDN 2010
;
; CATEGORY:
;
...
strout = ['PANSpec: Spectral Peak Analysis','Written by R.M.Dimeo', $
          'October 11, 2002','','NIST Center for Neutron Research', $
          '','Updated (RMD) May 11, 2005', $
	  'Updated (JDN) May 13, 2010']
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
pro opan::BlowUpErrors, event = event
; Blows up errors between vertical lines
self.addcurve = 5
if n_elements(*self.dataPtr) eq 0 then return
widget_control,self.info,set_value = 'Mark region to Blow Up'

return
end
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
pro opan::VelocityCoord, event = event
; toggles plotting velocity or input coordinate
widget_control,self.wave0Field,get_value = wave0
self.wave0 = float(wave0[0])
if self.plotvelocity eq 0 then begin
	self.plotvelocity = 1
	widget_control,self.info,set_value = 'Plotting Velocity [km/s]'
endif else begin
	self.plotvelocity = 0
	widget_control,self.info,set_value = 'Plotting Coordinate'
endelse
wset,self.datPix
self->displaydata
wset,self.datVis
...
if self.plotvelocity then begin
	widget_control,self.info,set_value = 'Fitting requires coord display'
	return
endif
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
pro opan::BlowUpEvents, event = event
case event.type of
...
1:	begin	; button release
	 if self.mouse eq 1 then begin
	  ...
	  xrange = [ll[0],ur[0]]
    	  daterr = reform(*self.errorPtr)
    	  x = reform(*self.xvalsPtr)
	  t=where(x ge min(xrange) and x le max(xrange),nt)
	  if nt gt 0 then begin
		daterr[t] = daterr[t]*1.e6
  	  	*self.errorPtr = daterr
	  endif
	  wset,self.datPix
	  self->displaydata
	  wset,self.datVis
	 ...
	 widget_control,self.info,set_value = $
		 strtrim(string(xrange[0],form='(f11.3)'),2) + ' - ' + $
		 strtrim(string(xrange[1],form='(f11.3)'),2) + $
		 ' errs * 1.e6'
	end
...
1:	begin	; button release
	 if self.mouse eq 1 then begin
	  ...
	  if self.plotvelocity then $
		  self.xrange = (self.xrange/299792.46+1.)*self.wave0
...
0:	begin		; button press
	  ...
	  if self.mouse eq 1 then begin
		...
		(*self.ocurveGroup)[val]->draw,overplot = 1, plotvelocity=self.plotvelocity,wave0=self.wave0
	  endif
	end
1:	begin	; button release
		dcoords = convert_coord(event.x,event.y,/device,/to_data)
...
if n_elements(*self.dataPtr) eq 0 then return
  ...
  if datSize[0] eq 2 then begin
    dat = (*self.dataPtr)[*,val]
    daterr = (*self.errorPtr)[*,val]
    xc = (*self.xvalsPtr)[*,val]
    y = (*self.yvalsPtr)[0,val]
  endif else begin
    dat = reform(*self.dataPtr)
    daterr = reform(*self.errorPtr)
    xc = reform(*self.xvalsPtr)
    y = 0  ;MSW: 30/1/09 Need this to load a file with only 1 spectrum. Hopefull this works in all cases
    val = 0
  endelse

thisFormat = '(f5.0)'
self.title = self.printFileName+'   !6Spectrum No = '+strtrim(string(y+1,format = thisformat),2)

good = where(daterr lt 1.e4, ngood)
if ngood le 0 then good = lindgen(n_elements(daterr))
if self.autoscale eq 1 then begin
  dx = 0.1*(max(xc)-min(xc))
  self.xrange = [min(xc)-dx,max(xc)+dx]
  dy = 0.1*(max(dat(good))-min(dat(good)))
  self.yrange = [min(dat(good)-daterr(good))-dy,max(dat(good)+daterr(good))+dy]
endif
...
if self.plotvelocity then begin
	x = (xc/self.wave0 - 1.) * 299792.46
	xrange = (self.xrange/self.wave0 - 1.) * 299792.46
...
plot,x,dat,xtitle = xtitle,ytitle = self.ztitle,title = self.title, $
     xrange = xrange,yrange = self.yrange,xstyle = 1,ystyle = 1, $
     charsi=1.25,_Extra = extra;,$
     ;psym = 4
...
if self.plotvelocity then begin
	xfitLo = (xfitLo/self.wave0 - 1.) * 299792.46
	xfitHi = (xfitHi/self.wave0 - 1.) * 299792.46
endif
xdispLo = (!x.crange)[0]
xdispHi = (!x.crange)[1]
if xfitLo gt xdispLo then $
  plots,[xfitLo,xfitLo],!y.crange,linestyle = 2,color=(256L*(255L+256L*255L))
if xfitHi lt xdispHi then $
  plots,[xfitHi,xfitHi],!y.crange,linestyle = 2,color=(256L*(255L+256L*255L))

if obj_valid((*self.ocurveGroup)[val]) then begin
  ncurves = (*self.ocurveGroup)[val]->count()
  ocurves = (*self.ocurveGroup)[val]
  ocurves->drawcomponents,plotvelocity=self.plotvelocity,wave0=self.wave0, $
	  linestyle = 2,thick = 1.5,color=255
  ocurves->evaluate,xc,yout = yout

  if n_elements(yout) gt 1 then begin
    oplot,x,yout,linestyle = 0,thick = 1.5,color=255
  endif else begin
...
if self.plotvelocity then begin
	...
	xtitle = self.xtitle
endelse
plot,x,residuals,psym = 0,xrange = xrange,xstyle = 1,xtitle = xtitle,$
     ytitle = '!6Residuals', charsi=1.25,_Extra = extra
plots,!x.crange,[1.0,1.0],/data,linestyle = 2,thick = 1.5,color=255
plots,!x.crange,[-1.0,-1.0],/data,linestyle = 2,thick = 1.5,color=255
return
end
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
; nice little if statement - more useful if it gave you the line in the code
; that screwed up though!
catch,theError
if theError ne 0 then begin
  catch,/cancel
  void = dialog_message(dialog_parent = event.top, $
...
for i = 0,ngroups-1 do begin
  ...
  if chisq ne (-1.0) then begin
    ncurves = oc->count()
    if ncurves ne 0 then begin
      spax_number = i + fix((*self.yvalsPtr)[0])
...
for i = 0,ngroups-1 do begin
  ...
  if chisq ne (-1.0) then begin
    ncurves = oc->count()
    if ncurves ne 0 then begin
      spax_number = i + fix((*self.yvalsPtr)[0]) + 1
      ; call displayparms_katrina procedure and pass the trans varaible and the group no
...
for i = 0,ngroups-1 do begin
  ...
  if ncurves gt 0 then begin
    ...
    thisFormat = '('+strtrim(string(4+ncurves),2)+'f17.5)'
    ...
    printf,lun,strout2,format = outFormat
    thisFormat = '('+strtrim(string(3),2)+'f17.5)'
    dataOut = fltarr(3)
    for j = 0,nx-1 do begin
...
if n_elements(title) eq 0 then title = 'PANSpec Log Output'
if n_elements(username) eq 0 then username = 'PANSpec User'
...
strout = [strout,'<meta name="author" content="PANSpec">']
strout = [strout,'<title>'+title+'</title>']
strout = [strout,'</head>']
strout = [strout,'<body>']
strout = [strout,'<big><big><b>PANSpec Data Analysis Log</b></big></big><br>']
strout = [strout,'<br>']
...
if self.group_leader eq (-1L) then begin
  tlb = widget_base(/row, $
        title = 'PANSpec: Spectral Peak Analysis',mbar = bar,/tlb_size_events)
endif else begin
...
void = widget_button(miscmenu,value = 'About PANSpec',$
...
void = widget_button(ctrlbase,value = 'Blow up errors', $
       uvalue = {object:self,method:'BlowUpErrors'})

self.wave0Field = cw_field(ctrlbase,/col,value = 1.,/string, $
                  title = 'Ref. coord for velocity')
void = widget_button(ctrlbase,value = 'Velocity/Coord', $
       uvalue = {object:self,method:'VelocityCoord'})
