; $Id: func_cont__define.pro,v 1.2 2010/05/14 17:29:25 neill Exp $
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
pro func_cont::drawComponents,plotvelocity=plotvelocity,wave0=wave0, $
	_Extra = extra
nfuncs = self->count()
...
for i = 0,nfuncs-1 do begin
  ...
  if (name ne 'pan_delta') and (n_elements(yvalues) gt 1) then begin
	  if keyword_set(plotvelocity) and keyword_set(wave0) then $
  		oplot,(xvalues/wave0-1.)*299792.46,yvalues,_Extra = extra $
  	  else	oplot,xvalues,yvalues,_Extra = extra
  endif
endfor
return
end
...
pro func_cont::draw,noerase = noerase,overplot = overplot,resPtr = resPtr, $
	plotvelocity=plotvelocity,wave0=wave0
nfuncs = self->count()
if nfuncs eq 0 then return
oall = self->get(/all)
...
if overplot ne 0 then begin
  if keyword_set(plotvelocity) and keyword_set(wave0) then $
       oplot,(xvalues/wave0-1.)*299792.46,yvalues,linestyle = 2, $
       		color=(255+256L*256L*255L) $
  else oplot,xvalues,yvalues,linestyle = 2,color=(255+256L*256L*255L)
endif else begin
  if keyword_set(plotvelocity) and keyword_set(wave0) then $
       plot,(xvalues/wave0-1.)*299792.46,yvalues,linestyle = 2, $
       		noerase = noerase, color=(255+256L*256L*255L) $
  else plot,xvalues,yvalues,linestyle = 2,noerase = noerase, $
  		color=(255+256L*256L*255L)
endelse
end
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
