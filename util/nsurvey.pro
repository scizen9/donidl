pro nsurvey, blink=blink, playback=playback, start=start, $
	scales=scales
;+
;	nsurvey - routine to display nova survey data and allow interactive
;		blinking and recording of nova candidates to OFILE.
;
; INPUTS/OUTPUTS:
;	data - returns image data so can restart without having
;		to read from disk
;
; KEYWORDS:
...
;	print,'NSURVEY: Usage - nsurvey, data, flist'
;	return
;endif
;
; check keywords
...
;stroff = strpos(tlist,'.')
;for i=0,ntim-1 do $
;	titles(i) = strmid(tlist(i),0,stroff(i)) 
;
; get scale factors
...
if nfiles le 0 then $			; no current file, so open a new one
	...
	if nin gt 0 then begin		; reformat vectors
		...
		cmntin = cmntin(1:nin)
					; print them out
		for i=0,nin-1 do print,xin(i),yin(i),tittin(i),cmntin(i), $
			format=cfmt
	endif
...
;
; check if we need to read data in
...
if play_back then begin			; start with first object
	...
	ix = grid(4,ip)
	iy = grid(5,ip)
					; report starting field
	print,'sub-field:'+string(ip+1,form='(i3)')+$
...
; check for marked objects in frame
	if nin gt 0 then begin		; objects were read in
		...
		tin = where(xin lt x1 and xin gt x0 and $
			  yin lt y1 and yin gt y0,nm)

 		if nm gt 0 then begin	; we've got some in the subfield
					; so print them out
			for i=0,nm-1 do $
				print,xin(tin(i)),yin(tin(i)),tittin(tin(i)), $
					cmntin(tin(i)), format=cfmt
		endif
			...
			'm':	begin	; mark an object
				...
				if strtrim(cmnt,2) ne '' then begin
					...
					printf,olun,xout,yout,titles(i),cmnt, $
						'('+strn(ip+1)+')', format=cfmt2
					plots,[x,x],[y,y],/dev,psym=8,symsiz=7
					xin=[xin,xout]
					yin=[yin,yout]
		...
		if q ne 'g' then begin	; go to next object/sub-frame
			...
			if play_back then begin	; next object
				if ip lt nin then begin
					...
					y1 = grd(3)
					print,' '
					print,'cand:'+string(ip+1,form='(i4)')+$
						'/'+string(nin,form='(i4)')
