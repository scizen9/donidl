Function adstrn,ra_dec,dec,precision, TRUNCATE = truncate, DELIM = delim
;+
; NAME:
;       ADSTRING
; PURPOSE:
...
;      /DELIM - if set use this character as a delimiter between the digits of
;		ra and dec in the string.  Default is to use colons.
; OUTPUT:
...
;-
  ...
  dlm=':'
  if keyword_set(DELIM) then dlm=delim
 ...
 if not keyword_set(truncate) then begin
    ...
    leadzero = replicate(dlm,N_elements(xsec))
    less10 = where(xsec LT (10.-10^(-float(precision+1))/2. ),Nzero)
    if Nzero GT 0 then leadzero[less10] = dlm+'0'
     result = string(ihr,'(I3.2)') + dlm + string(imin,'(I2.2)') + $
  ...
  endelse
   ...
   leadzero = replicate(dlm,N_elements(xsc))
   if precision NE 0 then begin
      less10 = where(xsc LT (10.- 10^(-float(precision))/2.),Nzero)
      if Nzero GT 0 then leadzero[less10] = dlm+'0'
   endif 

   return, result + deg + dlm + string(imn,'(I2.2)') + leadzero + $
