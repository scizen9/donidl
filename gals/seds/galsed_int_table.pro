pro galsed_int_table,t,samnam,saminfo,lem,testmags=testmags
;
; print out the following table:
;
;	galsed_int.txt	- integrated measures
common galdb_info
;
if n_params(0) lt 4 then begin
	print,'Usage: galsed_int_table, <indx_list>, <samnam>, <saminfo>, <limits_legend>, /testmags'
	return
endif
;
; get sample
...
;
; open output file
ofil='galsed_int_sam'+strtrim(samnam,2)+'.txt'
filestamp,ofil
openw,li,ofil,/get_lun
printf,li,'# GALSED_INT_TABLE: '+systime(0)
...
fmt='(a25,f9.1,34f9.2,f7.3,2x,a)'
;
; header
hdr='# ID                            cz     FUV      FUVe     NUV      NUVe      U        Ue       B        Be       V        Ve    u_sdss     usde   g_sdss     gsde   r_sdss     rsde   i_sdss     isde   z_sdss     zsde      J        Je       H        He       K        Ke      12mu    12mue     25mu    25mue     60mu    60mue    100mu   100mue  EBVMW  SNe'
printf,li,hdr
;
; loop over sample
for i=0,nsam-1 do begin
	...
	printf,li,galsam(i).id,galsam(i).cz, $
