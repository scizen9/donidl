pro plot_bestfit_zpeg_gals,ID,zpegres,$
                      _extra=_extra,error=error,fnu=fnu,nufnu=nufnu, $
		      reread=reread,ylog=ylog,$
                      zpeg_scen=zpeg_scen,onlyphot=onlyphot,mygalin=mygalin, $
		      nolegende=nolegende,$
                      index=index,myyr=myyr,psout=psout, $
                      xrange=xrange,savescen=savescen,unusedphot=unusedphot,$
                      flines_mod=flines_mod,lambdalines_mod=lambdalines_mod, $
		      residuals=residuals,singlefit=singlefit
;+
...
;
;-
; conversion from Angstroms to microns
tomic = 1.e4

  if (not(keyword_set(fnu)) and not(keyword_set(nufnu))) then $
	  flambda=1 else flambda=0
  ...
  zpeg_get_fitinfo,ID,zpegres,$
                   ...
                   error=error,fnu=fnu,nufnu=nufnu,reread=reread, $
		   zpeg_scen=zpeg_scen,mygalin=mygalin,index=index, $
		   savescen=savescen
  ...
  if keyword_set(psout) then begin
	  font_store=!p.font
	  if strlen(psout) le 0 then begin
	  	if keyword_set(singlefit) then $
			psfile,zpegres,/color $
	  	else	psfile,realid,/color
	  endif else 	psfile,psout,/color
	  !p.font=0
	  si=1.5
	  sl=1.25
	  th=5
...
; X RANGE --------------------------------------------------
  ...
  if (not(keyword_set(unusedphot))) then begin
     iok=where(pho_err(1,*) gt 0. and $
               stfilters.lambdamean ge xminmax(0) and $
...
; Y RANGE --------------------------------------------------
  ...
  plot,[0,1],[0,1],/nodata,xr=xminmax,yr=yminmax, $
       xtit='Lambda (A)',ytit=myytit,xs=1,ys=myys,title=realID, $
       ylog=ylog,charsize=si,xthick=th,ythick=th,charthi=th,_extra=_extra
  ...
  for i=0,nsols-1 do begin
     ...
     if not keyword_set(residuals) then begin
        ...
        for l=0,n_elements(lambdalines)-1 do begin
           ...
           oplot,lambdalines(l)*[1,1],flux_mod(iok,i)+ $
		   flines_mod(l,i)/200.*[0,1],$
        ...
        for ifi=0,nfilters-1 do begin
           if (not(keyword_set(onlyphot)) or $
               (keyword_set(onlyphot) and pho_err(0,ifi) gt 0.)) then $
                  oplot,[stfilters(ifi).lambdaeff],[models(ifi,i)],psym=4, $
		  color=pcolor,symsize=ss*2.0
        endfor        
     endif else begin
        oplot,[1,1d6],[0,0]
        for ifi=0,nfilters-1 do begin
           if (pho_err(0,ifi) gt 0) then begin ; filter used
              ...
              oploterror,[stfilters(ifi).lambdaeff], $
		      -[pho_obscalib(ifi)-modelmags(ifi,i)], $
		      [pho_obscalib_err(ifi)],psym=8,symsize=ss, $
		      color=mycolor,errcolor=mycolor
              oploterror,[stfilters(ifi).lambdaeff], $
		      -[pho_obscalib(ifi)-modelmags(ifi,i)], $
		      [modelmagerrs(ifi,i)],psym=8,symsize=ss, $
		      color=mycolor,errcolor=mycolor,hat=2.*!D.X_VSIZE / 100
              ...
              oplot,[stfilters(ifi).lambdaeff],$
                    ...
                    psym=4,symsize=ss,color=mycolor
              ...
              if keyword_set(unusedphot) then begin
                 ...
                 oplot,[stfilters(ifi).lambdaeff], $
			 -[pho_obscalib(ifi)-modelmags(ifi,i)], $
			 psym=8,symsize=ss,color=mycolor
                 oploterror,[stfilters(ifi).lambdaeff], $
			 -[pho_obscalib(ifi)-modelmags(ifi,i)], $
			 [abs(modelmagerrs(ifi,i))],psym=8,symsize=ss, $
			 color=mycolor,errcolor=mycolor,hat=2.*!D.X_VSIZE / 100
              endif
           endelse       
        endfor
...
;                               mygal.zphots(i).z(0)-min(mygal.zphots(0:mygal.nsol-1).z(1)),$
                ...
                linestylep1=1,color=mycolor,charsize=sl,chcolor=0,linelen=0.7,thick=2.,_extra=_extra
  ...
  if not keyword_set(residuals) then begin
     for i=0,nfilters-1 do begin
        if (pho_err(0,i) gt 0) then begin
           vsym,8,/fill
           oploterror,[stfilters(i).lambdaeff], $
		   [pho(i)],[pho_err(0,i)],psym=8,symsize=ss,/lobar, $
		   color=colordex('R')
           oploterror,[stfilters(i).lambdaeff], $
		   [pho(i)],[pho_err(1,i)],psym=8,symsize=ss,/hibar, $
		   color=colordex('R')
        endif else begin
           if keyword_set(unusedphot) then begin
              vsym,8
              oplot,[stfilters(i).lambdaeff],[pho(i)],psym=8,symsize=ss
           endif
        endelse
     endfor
  endif
