	pro ketc, h=h, slicer, grating, grat_wave, f_lam_index, seeing, exposure_time, ccd_bin,  mag_AB=mag_AB, flux=flux, Nframes=Nframes, nas=nas, $
...
;
	...
	Nf = 1.
	if keyword_set(Nframes) then Nf = Nframe
	...
	if grating eq 'BL' then begin
		A_per_pixel = 0.625
	endif
	if grating eq 'BM' then begin
		A_per_pixel = 0.28
	endif
	if grating eq 'BH2' or grating eq 'BH3' then begin
		A_per_pixel = 0.125
	endif
...
;	1 pixel spectral depends on grating: slicer BL L = 8 CCD pixels per spectral element = 4500/900 = 5Å; M = 2.5Å; S = 1.25Å
	...
	dy = (y1-y0)/6.
	chsz=1.
	window,0,title='SNR',ysize=1100
        plot,w,snr,ytitle='SNR /'+string(format='(f4.1)',snr_spectral_bin)+' Å',charsize=chsz,title=plotout,pos=[0.15,y1-dy,0.9,y1]
        plot,w,c_o,ytitle='Obj cts /'+string(format='(f4.1)',snr_spectral_bin)+' Å',charsize=chsz,pos=[0.15,y1-2*dy,0.9,y1-dy],/noerase
        plot,w,c_s,ytitle='Sky cts /'+string(format='(f4.1)',snr_spectral_bin)+' Å',charsize=chsz,pos=[0.15,y1-3*dy,0.9,y1-2*dy],/noerase
        plot,w,c_r*replicate(1.,n_elements(w)),ytitle='Read Noise cts /'+string(format='(f4.1)',snr_spectral_bin)+' Å',charsize=chsz,pos=[0.15,y1-4*dy,0.9,y1-3*dy],/noerase,yrange=[0.,2.*c_r]
        plot,w,c_o/c_s,ytitle='Obj cts / Sky cts',charsize=chsz,pos=[0.15,y1-5*dy,0.9,y1-4*dy],/noerase
