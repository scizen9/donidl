pro sn_rgb_sdss_asinh, fspec=fspec,scales=scales,brite=brite, $
                     nonlinearity=nonlinearity, origin=origin, $
                     verbose=verbose, quality=quality, $
		     outdir=outdir, plotaps=plotaps, update=update
;+
; NAME:
;  sn_rgb_sdss_asinh
;
...
;
; CALLING SEQUENCE:
;  sn_rgb_sdss_asinh, fspec=fspec,scales=scales,brite=brite, $
;                     nonlinearity=nonlinearity, origin=origin, $
;                     verbose=verbose, quality=quality
;
; INPUTS:
...
for i=0,n_elements(pfile)-1 do begin 
 ...
 gfile=pfile[i]
 strput,gfile,'g',strpos(pfile[i],'z')
 rfile=pfile[i]
 strput,rfile,'r',strpos(pfile[i],'z')
 ifile=pfile[i]
 ...
 if be or re or gg then begin  ; if fits files exists
  ...
  if be     then  begin
    blue = mrdfits(gfile,0,bh,/fscale,/silent)+0.0002
    hdr = bh
  endif
  if gg     then  begin
    green  = mrdfits(rfile,0,gh,/fscale,/silent)+0.0002
    hdr = gh
  endif
  if re     then  begin
    red  = mrdfits(ifile,0,rh,/fscale,/silent)+0.0002
    hdr = rh
  endif
  ...
  host=strmid(root,0,strpos(root,'_z'))
  h=snhfind(host,nsn)
  ;
  ; loop over sne
...
;
  ...
  jname=odir+host+'_'+snnm+sfx+'_sdss.jpg'
