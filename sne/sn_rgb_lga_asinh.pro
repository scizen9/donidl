pro sn_rgb_lga_asinh, fspec=fspec,scales=scales,brite=brite, $
...
;
; CALLING SEQUENCE:
;  sn_rgb_lga_asinh, fspec=fspec,scales=scales,brite=brite, $
;                     nonlinearity=nonlinearity, origin=origin, $
;                     verbose=verbose, quality=quality,tif=tif, $
;		     plotaps=plotaps
;
...
;
; get file list
if not keyword_set(fspec) then fspec='*prof.ps'
...
; print,'Usage: sn_rgb_lga_asinh, sn_name, galaxy_name'
; return
;endif
...
for i=0,n_elements(pfile)-1 do begin 
 ...
 sta=strsplit(pfile[i],'_',/extract)
 hdir=sta(0)
 snnm=sta(1)
 sta=strsplit(hdir,'/',/extract)
 host=sta(n_elements(sta)-1)
 jfile=hdir+'_mosaic_j.fits'
 hfile=hdir+'_mosaic_h.fits'
 kfile=hdir+'_mosaic_k.fits'
 ...
 if be or re or gg then begin  ; if fits files exists
  ...
  if be     then  begin
    blue = mrdfits(jfile,0,bh,/silent)
    ;blue= blue*1.4*1525  ; scale as lambda*flambda
    hdr = bh
  endif
  if gg     then  begin
    green  = mrdfits(hfile,0,gh,/silent)
    ;red=  red*0.206*2297
    hdr = gh
  endif
  if re     then  begin
    red  = mrdfits(kfile,0,rh,/silent)
    ;red=  red*0.206*2297
    hdr = rh
  endif
...
;
; get aperture sizes (scale for 2mass LGA is 1 arcsec per pixel)
