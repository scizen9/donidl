pro twomass_mega_driver,lfile,clean=clean,pxscl=pxscl,nolimit=nolimit, $
	noskytweak=noskytweak, update=update, shrink=shrink, $
	fast=fast,medium=medium,slow=slow
;+
; twomass_mega_driver - create coadded twomass images
...
;
; directories
root = !GLGA_2MASS_DATA + 'data/sort/'
;
; loop over hosts
ng=n_elements(names)
for ig=0,ng-1 do begin
    ...
    if file_test(dir,/directory) then begin
      ...
      if not file_test(dir+names[ig]+'_j.fits*') or $
	...
	if file_test('?i???????.fits.gz') then begin
		;
		...
		imsize = max(mosaic_max_imsize('./','k*fit*',/verbose))
		;
		...
		for iband=0,nband-1 do begin
			...
			mimap_2mass_combine,'',band[iband], $
				names[ig]+'_'+strlowcase(band[iband])+'.fits', $
			...
			if dorebin then begin
				; image file
				imfl = names[ig]+'_'+strlowcase(band[iband]) + $
					'.fits'
