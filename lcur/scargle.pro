PRO scargle,t,c,om,px,fmin=fmin,fmax=fmax,nif=nif,	$
...
;
; CALLING SEQUENCE:
;   scargle,t,c,om,px,fmin=fmin,fmax=fmax,nif=nif,pmin=pmin,pmax=pmax, 
;            nu=nu,period=period,omega=omega,fap=fap,signi=signi
;
; 
; INPUTS:
;         t: The times at which the time series was measured
;         c: the corresponding count rates
;
;
; OPTIONAL INPUTS:
...
;         nif: number of independent frequencies
;      
;   
; KEYWORD PARAMETERS:
...
;                    noise simulations
;            psdpeaksort : array with the maximum peak probability for
;		     each simulation    
;
; PROCEDURE:
;         The Lomb Scargle PSD is computed according to the
;         definitions given by Scargle, 1982, ApJ, 263, 835, and Horne
;         and Baliunas, 1986, ApJ, 302, 757. Beware of patterns and
...
;          Version 1.2, 1999.01.07, JW: force nif to be int
...
;-
   ...
   IF (n_elements(nif) EQ 0) THEN nif = horne ELSE horne = nif
   ...
   IF (n_elements(omega) EQ 0) THEN BEGIN 
       om = 2.D0 * !DPI* (fmin+(fmax-fmin)*findgen(nif)/(nif-1.D0))
   END ELSE BEGIN 
       om = omega
   END
   
   ;; False Alarm Probability according to Nif
