pro irafwrite, image, hd, filename
;+
; NAME:
;     IRAFWRITE
; PURPOSE:
...
; EXPLANATION:
;     Does the reverse of IRAFRD.    IRAFWRITE writes the "old" IRAF format
;     used prior to v2.11.   However, this "old" format is still readable by
;     the current version of IRAF.
;
; CALLING SEQUENCE: 
;    IRAFWRITE, image, hdr, filename, [ PIXDIR = ]
;
; INPUTS:
...
;
; PROCEDURE:
;       IRAFWRITE gets information about the data - image dimensions, size, 
...
;
; RESTRICTIONS:
;       (1) The files are not created by IRAFWRITE are not identical to those 
;               created by the IRAF routine rfits.    However, the files 
;               created by IRAFWRITE appear to be compatible with all the IRAF 
;               routines tested so far.
;       (2)  IRAFWRITE has been tested on a limited number of data types
;       (3)  IRAFWRITE has only been tested on Unix and VMS systems.
;
; PROCEDURES CALLED:
...
;-     
 ...
 if N_params() LT 3 then begin
        print,'Syntax - IRAFWRITE, image, header, filename, [PIXDIR = ]'
        return
  endif
;
; Get the dimensions, vector of dimensions and the data type
 ...
 object = sxpar( hd, 'OBJECT',/silent)
; if ( !ERR EQ -1 ) or ( object EQ '' ) then object = ' '
if ( object EQ '' ) then object = ' '
 origin = sxpar( hd, 'ORIGIN',/silent)
; if ( !ERR EQ -1 ) or ( origin EQ '') then origin = ' '
if ( origin EQ '') then origin = ' '
 date_obs = sxpar( hd, 'DATE-OBS',/silent) 
; if ( !ERR EQ -1 ) or ( date_obs EQ '')  then date_obs = ' '
if ( date_obs EQ '')  then date_obs = ' '
