...
; KEYWORDS
;   nocleanout  Don't output the cleaned image.  If you only want the
;                masks.
; OUTPUTS:
;
; OPTIONAL OUTPUTS:
...
pro la_cosmic, imlist, outlist=outlist, masklist=masklist, sigclip=sigclip, $
               ...
               niter=niter,sigfrac=sigfrac, verbose=verbose, $
               statsec=statsec,zeroindexed=zeroindexed,$
               masksuffix=masksuff, outsuff=outsuff, isbig=isbig, $
               blocksize=blocksize, NOCLEANOUT=nocleanout

IF KEYWORD_SET( nocleanout ) THEN outputclean=0b ELSE outputclean=1b
...
for i=0, nfiles -1 do begin
    ...
    while(not sstop) do begin
        ...
        if (skyval[i] NE 0.0) then oldoutput = temporary(oldoutput) + skyval[i]
...
;              PRINT,regx[xchop,0],regx[xchop,1]
;              PRINT,regy[ychop,0],regy[ychop,1]
